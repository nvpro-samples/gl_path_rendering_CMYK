...
bool GLSLProgram::setIncludeFromFile(const char *includeName, const char* filename)
{
    ...
    glNamedStringARB(GL_SHADER_INCLUDE_ARB, GLint(strlen(tmpstr)), tmpstr, GLint(strlen(incStr)), incStr);
    return false;
}
void GLSLProgram::setIncludeFromString(const char *includeName, const char* str)
{
    char tmpstr[200];
    sprintf(tmpstr, "/%s", includeName);
    glNamedStringARB(GL_SHADER_INCLUDE_ARB, GLint(strlen(tmpstr)), tmpstr, GLint(strlen(str)), str);
}
